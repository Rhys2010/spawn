...
function Login.NUICallback(data)
	...
	if data.action == "newCharacter" then
		...
		events:Trigger("np-base:createCharacter", chardata, function(created)
            if not created then
				TriggerEvent("DoLongHudText","An error occurred while creating your character, the value was returned as zero or false. If this situation persists, contact an administrator.",2) 
